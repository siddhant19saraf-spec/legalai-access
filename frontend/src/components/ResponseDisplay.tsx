...
const RISK_CONFIG: Record<RiskLevel, { bg: string; border: string; icon: string; text: string; label: string; ring: string }> = {
  low: { bg: 'bg-green-50', border: 'border-green-200', icon: 'text-green-600', text: 'text-green-800', label: 'Low Risk', ring: 'focus:ring-green-500' },
  medium: { bg: 'bg-yellow-50', border: 'border-yellow-200', icon: 'text-yellow-600', text: 'text-yellow-800', label: 'Medium Risk', ring: 'focus:ring-yellow-500' },
  high: { bg: 'bg-orange-50', border: 'border-orange-200', icon: 'text-orange-600', text: 'text-orange-800', label: 'High Risk', ring: 'focus:ring-orange-500' },
  critical: { bg: 'bg-red-50', border: 'border-red-200', icon: 'text-red-600', text: 'text-red-800', label: 'Critical Risk', ring: 'focus:ring-red-500' },
};
...
function SectionHeader({ number, title, children, className = '' }: { number: string; title: string; children?: React.ReactNode; className?: string }) {
  return (
    <div className={`mb-4 ${className}`}>
      <div className="flex items-start gap-3 mb-2">
        <span className="flex-shrink-0 w-8 h-8 rounded-lg bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 text-sm font-semibold flex items-center justify-center border border-blue-200 dark:border-blue-800" aria-hidden="true">
          {number}
        </span>
        <div>
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      ...
      <div className="p-5 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
        <p className="text-gray-800 dark:text-gray-200 leading-relaxed">{response.summary}</p>
      </div>
      {response.explanation && (
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">Detailed Explanation</h4>
...
function QuestionQualitySection({ quality }: { quality: QuestionQuality | undefined }) {
  ...
  return (
    <div className="space-y-3 animate-slide-up">
      ...
      <div className={`p-4 rounded-xl border ${config.bg} ${config.border} dark:bg-opacity-20`} role="status">
        <div className="flex items-center gap-3">
          <span className={`font-semibold ${config.text}`}>{config.label}</span>
          <span className="text-sm text-gray-600 dark:text-gray-400 font-mono">({quality.score}/100)</span>
        </div>
        {quality.missing_information.length > 0 && (
          <div className="mt-3">
            <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Useful details that could improve the response:</h4>
        ...
        <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">You can still continue with your question even if more context would help.</p>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-4 animate-slide-up">
      ...
      <div className="p-4 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl space-y-3">
        <div className="flex items-start gap-3">
          <span className="flex-shrink-0 font-medium text-blue-600 dark:text-blue-400 w-32">Detected topic:</span>
          <span className="text-gray-700 dark:text-gray-300">{response.legal_category.replace('_', ' ').toUpperCase()}</span>
        </div>
        <div className="flex items-start gap-3">
          <span className="flex-shrink-0 font-medium text-blue-600 dark:text-blue-400 w-32">Response mode:</span>
        ...
        <div className="flex items-start gap-3">
          <span className="flex-shrink-0 font-medium text-blue-600 dark:text-blue-400 w-32">Source coverage:</span>
          <span className="text-gray-700 dark:text-gray-300">
        ...
        <div className="flex items-start gap-3">
          <span className="flex-shrink-0 font-medium text-blue-600 dark:text-blue-400 w-32">Risk classification:</span>
          <span className="text-gray-700 dark:text-gray-300">{response.risk_level.replace('_', ' ').toUpperCase()}</span>
        </div>
        {response.sources.length > 0 && (
          <div className="flex items-start gap-3">
            <span className="flex-shrink-0 font-medium text-blue-600 dark:text-blue-400 w-32">Sources used:</span>
            <span className="text-gray-700 dark:text-gray-300">{response.sources.length} verified source{response.sources.length !== 1 ? 's' : ''}</span>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="space-y-3 animate-slide-up">
      ...
      <ul className="space-y-2" role="list">
        {steps.map((step, index) => (
          <li key={index} className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-700">
            <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-medium flex items-center justify-center mt-0.5 border border-blue-200 dark:border-blue-800">
              {index + 1}
            </span>
            <p className="text-sm text-gray-700 dark:text-gray-300 mt-0.5">{step}</p>
          </li>
        ))}
      </ul>
    </div>
...
function RiskSection({ riskLevel }: { riskLevel: RiskLevel }) {
  ...
  return (
    <div className="animate-slide-up">
      ...
      <div className={`p-4 rounded-xl border ${config.bg} ${config.border} dark:bg-opacity-20`} role="status">
        <div className="flex items-center gap-3">
          <svg className={`w-6 h-6 ${config.icon}`} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
          <span className={`font-semibold ${config.text}`}>{config.label}</span>
        </div>
        {riskLevel === 'high' || riskLevel === 'critical' ? (
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            Professional legal assistance may be appropriate for this situation.
          </p>
        ) : null}
      </div>
    </div>
...
function JurisdictionSection({ jurisdiction }: { jurisdiction: string }) {
  return (
    <div className="animate-slide-up">
      ...
      <div className="inline-flex items-center px-4 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{jurisdiction.replace('_', ' ').toUpperCase()}</span>
      </div>
    ...
    return (
      <div className="animate-slide-up">
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl">
          <p className="text-sm text-gray-600 dark:text-gray-400">No verified source was returned for this response.</p>
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">The response is based on general legal knowledge.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="space-y-3" role="list">
        {sources.map((source, index) => (
          <article
            ...
            className="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl hover:border-blue-300 dark:hover:border-blue-700 transition-colors"
            role="listitem"
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 border border-blue-200 dark:border-blue-800">
                    {SOURCE_TYPE_LABELS[source.type] || source.type}
                  </span>
                  {source.verified && (
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-50 dark:bg-green-900/30 text-green-800 dark:text-green-200 border border-green-200 dark:border-green-800">
                      Verified
                    </span>
                  )}
                  <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-600">
                    {source.jurisdiction.replace('_', ' ').toUpperCase()}
                  </span>
                </div>
                <h4 className="text-sm font-semibold text-gray-900 dark:text-white">{source.title}</h4>
                {source.citation && (
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-400 font-mono">{source.citation}</p>
                )}
                {source.excerpt && (
                  <p className="mt-2 text-sm text-gray-700 dark:text-gray-300 italic border-l-2 border-gray-200 dark:border-gray-700 pl-3">
                ...
                {source.url && (
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 underline mt-2"
                  >
...
function DocumentChecklistSection({ items }: { items: DocumentChecklistItem[] }) {
  ...
  return (
    <div className="space-y-3 animate-slide-up">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">Gathering these documents may help clarify your situation:</p>
      <ul className="space-y-2" role="list">
        {items.map((item, index) => (
          <li key={index} className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-700 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors" onClick={() => toggleItem(index)}>
            <input
              type="checkbox"
            ...
            />
            <div>
              <p className={`text-sm ${checkedItems.has(index) ? 'text-gray-500 line-through' : 'text-gray-700 dark:text-gray-300'}`}>
                {item.item}
              </p>
...
function TerminologyExplainerSection({ explanations }: { explanations: TerminologyExplanation[] }) {
  ...
  return (
    <div className="space-y-3 animate-slide-up">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">Some legal terms may appear in this response. Click to understand:</p>
      <div className="flex flex-wrap gap-2">
        {explanations.map((item) => (
          <button
            ...
            className="inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium bg-indigo-50 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-200 border border-indigo-200 dark:border-indigo-800 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
      ...
      {expandedTerms.size > 0 && (
        <div className="space-y-2 mt-3" role="list">
          {explanations.filter(e => expandedTerms.has(e.term)).map((item) => (
            <div key={item.term} className="p-3 bg-white dark:bg-gray-800 border border-indigo-200 dark:border-indigo-800 rounded-lg" role="listitem">
              <div className="flex items-center gap-2">
                <h4 className="text-sm font-semibold text-indigo-900 dark:text-indigo-200">{item.term}</h4>
                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                  {item.category}
                </span>
              </div>
              <p className="mt-1 text-sm text-gray-700 dark:text-gray-300">{item.explanation}</p>
            </div>
          ))}
        </div>
      )}
...
function NextStepsSection({ steps }: { steps: string[] }) {
  ...
  return (
    <div className="space-y-3 animate-slide-up">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">These steps are informational and safe:</p>
      <ol className="space-y-2" role="list">
        {steps.map((step, index) => (
          <li key={index} className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-700 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors" onClick={() => toggleStep(index)}>
            <input
              type="checkbox"
            ...
            />
            <p className={`text-sm ${checkedSteps.has(index) ? 'text-gray-500 line-through' : 'text-gray-700 dark:text-gray-300'} mt-0.5`}>
              {step}
            </p>
          </li>
        ))}
...
function FollowUpSection({ suggestions }: { suggestions: FollowUpSuggestion[] }) {
  ...
  return (
    <div className="space-y-3 animate-slide-up">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">These may help you explore further:</p>
      <div className="space-y-2">
        {suggestions.map((suggestion, index) => (
          <button
            key={index}
            type="button"
            className="block w-full text-left p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500"
            onClick={() => {
          ...
          >
            <p className="text-sm font-medium text-gray-900 dark:text-white">{suggestion.question}</p>
  ...
  return (
    <div className="pt-4 border-t border-gray-200 dark:border-gray-700 animate-slide-up">
      <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl">
        <div className="flex items-start gap-3">
          <svg className="flex-shrink-0 mt-0.5 h-5 w-5 text-amber-600 dark:text-amber-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
          <div>
            <h4 className="text-sm font-semibold text-amber-800 dark:text-amber-200">Legal Information Disclaimer</h4>
            <p className="mt-1 text-sm text-amber-700 dark:text-amber-300">{disclaimer}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="animate-slide-up">
      ...
      <div className={`p-4 rounded-xl border ${
        ...
        <div className="flex items-center gap-3">
          <span className={`font-semibold ${isLocal ? 'text-purple-800 dark:text-purple-200' : 'text-green-800 dark:text-green-200'}`}>
            {isLocal
    ...
    <div className="space-y-6" role="region" aria-label="Legal information response">
      <header className="space-y-4 pb-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Legal Information</h2>
          <div className="flex items-center gap-2 flex-wrap">
            <MetadataBadges response={response} />
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <span className="inline-flex items-center px-2.5 py-1 rounded text-xs font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
            <svg className="w-3 h-3 mr-1" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              <path d="M10 2a6 6 0 00-6 6v3.586l-.707.707A1 1 0 004 14h12a1 1 0 00.707-1.707L16 11.586V8a6 6 0 00-6-6zm0 16a3 3 0 01-3-3h6a3 3 0 01-3 3z" />
            </svg>
            AI-generated
          </span>
          <span className="text-xs text-gray-500 dark:text-gray-400">{response.generated_at}</span>
        </div>
      </header>
      ...
      <section aria-labelledby="feedback-heading" className="pt-6 border-t border-gray-200 dark:border-gray-700">
        <h3 id="feedback-heading" className="text-lg font-semibold text-gray-900 dark:text-white">Was this helpful?</h3>

        {feedbackSent ? (
          <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl text-green-800 dark:text-green-200" role="status">
          ...
          <form onSubmit={handleFeedbackSubmit} className="space-y-4">
            ...
            <div>
              <label htmlFor="feedback-comment" className="label block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-2.5 text-sm border rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
            ...
            {feedbackError && (
              <div role="alert" className="p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-sm text-red-800 dark:text-red-200">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                ...
                className="inline-flex items-center px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
              >
